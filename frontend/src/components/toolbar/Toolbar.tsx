...
export const Toolbar = ({ disabled = false }) => {
  ...
  React.useEffect(() => {
    ...
    return () =>
      persistance.persistor?.onPersistorStateChangeRemove(handleStatusChange);
  ...
  return (
    <div className={`${style.toolbar} ${disabled ? style.disabled : ""}`}>
      <div>
        <span onClick={toggleEditor} className={style.icon}>
          <EditIcon />
        </span>
      </div>
      <div>
        <span className={style.icon}>
          <span className={style.moon} onClick={() => setTheme("dark")}>
            <MoonIcon />
          </span>
          <span className={style.sun} onClick={() => setTheme("light")}>
            <SunIcon />
          </span>
        </span>
        <span className={style.icon} onClick={store.download}>
          <DownloadIcon />
        </span>
        <span
          onClick={() => {
        ...
        <span
          className={style.icon}
          title={persistance.persistor?.lastSync?.toTimeString()}
          onClick={persistance.store}
        >
          <SyncIcon isActive={isSyncing} />
        </span>
      </div>
    </div>
  );
